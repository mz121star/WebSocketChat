    ...
    public class WebSocketServer : IDisposable
    {
        ...
        void socketConn_Disconnected(Object sender, EventArgs e)
        {
            ...
            if (sConn != null)
            {
                ...
                if (connectionSocketList.Count != 0)
                    Send(ChatServerCore.GetUserList(connectionSocketList));
            }
        ...
        void socketConn_BroadcastMessage(Object sender, string message, EventArgs e)
        {
            ...
            if (message.IndexOf("login:") != -1)
            {
                ...
                message = string.Format("系统消息：欢迎【{0}】来到聊天室！", message.Substring(message.IndexOf("login:") + "login:".Length));

            }
            var chatmessage = new ChatMessage { MessageContent = message ,FromName =  sConn.Name};
            Send(Env.jsonSerializer.Serialize(chatmessage));
            Send(ChatServerCore.GetUserList(connectionSocketList));
